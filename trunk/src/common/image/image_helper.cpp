...
#undef IMAGE_HELPER_ERROR_TYPE_CASE
		...
		int ImageHelper::build(char* buffer, size_t buffer_len)
		{
			...
			try
			{
				...
				LOG4CPLUS_DEBUG(logger, "Magick::Blob create ok, buffer.size="<<_blen);
				
				Magick::Image image;
				image.read(r_blob);
				LOG4CPLUS_DEBUG(logger, "Magick::Image read ok");
				
				_image_width = image.columns();
				_image_heigth = image.rows();
				std::string format = image.magick();
				LOG4CPLUS_DEBUG(logger, "ImageHelper::build, format="<<format<<", _image_width="<<_image_width<<", _image_heigth="<<_image_heigth);
			...
			{
				LOG4CPLUS_DEBUG(logger, "ImageHelper::build, caught exception="<<e.what());
				return E_INVALID_CONTENT;
			}
			catch(...)
			{
				LOG4CPLUS_DEBUG(logger, "ImageHelper::build, unknown error");
				return E_INVALID_CONTENT;
			}
