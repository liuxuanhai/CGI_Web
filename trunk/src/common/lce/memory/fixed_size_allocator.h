...
namespace lce
{
    namespace memory
    {
        ...
        public:
            ...
            /*
             * allocate pages and save the start addresses of each page at the spaces of last page (if enough) or a new page (if not)
             */
             int allocate_block(void*& base, uint32_t total_block_num, uint32_t block_size)
             {
                ...
                if (last_page_free_size >= (page_num + 1) * sizeof(void *) && last_page_used != 0)
                {
                    ...
                    ::memset(block_ptr, 0, sizeof(void *));
                    block_ptr -= sizeof(void *);
                    ::memcpy(block_ptr, &curr, sizeof(void *));
            ...
            /*
             * support allocate small pages in a page
             */
