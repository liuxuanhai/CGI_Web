...
std::string valueToQuotedString(const char *value) {
	...
	if (strpbrk(value, "\"\\\b\f\n\r\t") == NULL && !containsControlCharacter(
			value))
...
bool StyledWriter::isMultineArray(const Value &value) {
	...
	for (int index = 0; index < size && !isMultiLine; ++index) {
		const Value &childValue = value[index];
		isMultiLine = isMultiLine || ((childValue.isArray()
				|| childValue.isObject()) && childValue.size() > 0);
	}
...
bool StyledWriter::hasCommentForValue(const Value &value) {
	return value.hasComment(commentBefore) || value.hasComment(
			commentAfterOnSameLine) || value.hasComment(commentAfter);
}
