...
#define s_env_home_path "MARK_SERVER_HOME_PATH"
...
FsmContainer<UserMarkFsm>*      g_user_mark_fsm_container  = NULL;
...
int Server::impl_init()
{
	...
	g_user_mark_fsm_container = new FsmContainer<UserMarkFsm>(stConfig.get_int_param("FSM_CONTAINER", "capacity"));
	if(g_user_mark_fsm_container->init() < 0)
	{
		cerr<<"user fsm container init failed !"<<endl;
		return -1;
	}
    assert(g_user_mark_fsm_container);
