...
#define s_env_home_path "TRANSCODE_SERVER_HOME_PATH"
...
FsmContainer<FfmpegFsm>*      g_ffmpeg_fsm_container  = NULL;
...
int Server::impl_init()
{
	...
	g_ffmpeg_fsm_container = new FsmContainer<FfmpegFsm>(stConfig.get_int_param("FSM_CONTAINER", "capacity"));
	if(g_ffmpeg_fsm_container->init() < 0)
	{
		cerr<<"user fsm container init failed !"<<endl;
		return -1;
	}
    assert(g_ffmpeg_fsm_container);
