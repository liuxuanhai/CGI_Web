...
IMPL_LOGGER(TokenServerProcessor, logger);

TokenServerProcessor::TokenServerProcessor()
{
}
...
void TokenServerProcessor::accept(lce::net::ConnectionInfo & conn)
{
...
void TokenServerProcessor::remove(lce::net::ConnectionInfo & conn)
{
	ConnMapIterator iter = _conn_map.find(conn.get_id());
	if(iter != _conn_map.end())
	{
...
void TokenServerProcessor::process_input(lce::net::ConnectionInfo& conn, lce::net::IDatagram& ogram )
{
	...
	{
		LOG4CPLUS_ERROR(logger, "TokenServerProcessor::process_input fail"
...
#undef CLIENT_PROCESSOR_CMD_HANDLER
		...
		default:
			LOG4CPLUS_ERROR(logger, "TokenServerProcessor::process_input fail, unknown cmd="<<dwCmd);
			conn.close();
			break;
	}
...
void TokenServerProcessor::process_pa_token_notify(lce::net::ConnectionInfo& conn, const ::hoosho::msg::Msg& msg)
{
...
void TokenServerProcessor::set_pa_info(const std::string& strAppid, const std::string& strBaseAcessTokenn, const std::string& strJSAPITicket)
{
	_base_access_token_map[strAppid] = strBaseAcessTokenn;
	_jsapi_ticket_map[strAppid] = strJSAPITicket;
}

int TokenServerProcessor::get_pa_info(const std::string& strAppid, std::string& strBaseAcessTokenn, std::string& strJSAPITicket)
{
	...
	{
		LOG4CPLUS_ERROR(logger, "TokenServerProcessor::get_pa_info failed, pa token info not exists for strAppid="<<strAppid
...
void TokenServerProcessor::process_output()
{
