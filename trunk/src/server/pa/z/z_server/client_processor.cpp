...
int ClientProcessor::load_zombie()
{
	std::ostringstream oss;
	oss.str("");
	oss<<"SELECT * FROM "<<m_db_table_user_info_name
	   <<" WHERE user_type="<<USER_TYPE_ZOMBIE;

	if(!m_mysql_helper.Query(oss.str()))
	{
		LOG4CPLUS_FATAL(logger, "ClientProcessor::load_zombie failed, error: "<<m_mysql_helper.GetErrMsg());
		return -1;
	}
	...
	if(m_mysql_helper.GetRowCount())
	{
		while(m_mysql_helper.Next())
		{
			...
			UserInfoBD2PB(m_mysql_helper, stUserInfo);
			add_login_session(stUserInfo.openid(), stUserInfo, DATE21160101);
		}
	}
...
void ClientProcessor::process_input(lce::net::ConnectionInfo& conn, lce::net::IDatagram& ogram )
{
	...
	if(stMsg.head().cmd() != ::hoosho::msg::Z_PROJECT_REQ)
	{
		LOG4CPLUS_ERROR(logger, "ClientProcessor::process_input fail, cmd: "<<stMsg.head().cmd()<<" != ::hoosho::msg::Z_PROJECT_REQ");
		conn.close();
		return;
	}	
		
	::hoosho::msg::z::MsgReq stZMsgReq = stMsg.z_msg_req();
	uint32_t dwSubCmd = stZMsgReq.sub_cmd();
	switch(dwSubCmd)
	{
		case ::hoosho::msg::z::GET_PA_JSAPI_SIGNATURE_REQ:
			return process_get_jsapi_ticket(conn, stMsg);

		case ::hoosho::msg::z::GET_PA_ACCESS_TOKEN_REQ:
			return process_get_access_token(conn, stMsg);

		case ::hoosho::msg::z::USER_LOGIN_REQ:
			return process_user_login(conn, stMsg);

		case ::hoosho::msg::z::WX_PAY_CALLBACK_NTF:
			return process_wx_pay_callback(conn, stMsg);
...
void ClientProcessor::process_get_jsapi_ticket(lce::net::ConnectionInfo& conn,  const ::hoosho::msg::Msg& msg)
{
	...
	pRespMsgHead->set_cmd(::hoosho::msg::Z_PROJECT_RES);
	...
	//req invalid
	std::string strAppid = stZMsgReq.get_pa_jsapi_sign_req().pa_appid();
	std::string strReqUrl = stZMsgReq.get_pa_jsapi_sign_req().req_url();
	...
	//build jsapi signature
	{
		...
		pZMsgRes->set_sub_cmd(::hoosho::msg::z::GET_PA_JSAPI_SIGNATURE_RES);
		
		::hoosho::msg::z::GetPAJSAPISignRes* pRespBody = pZMsgRes->mutable_get_pa_jsapi_sign_res();
...
void ClientProcessor::process_get_access_token(lce::net::ConnectionInfo& conn,  const ::hoosho::msg::Msg& msg)
{
	...
	//req invalid
	std::string strAppid = stZMsgReq.get_pa_access_token_req().pa_appid();
	strAppid = lce::util::StringOP::Trim(strAppid);
	if(strAppid.empty())
	{
	...
	::hoosho::msg::z::MsgRes* pZMsgRes = stRespMsg.mutable_z_msg_res();
	pZMsgRes->set_sub_cmd(::hoosho::msg::z::GET_PA_ACCESS_TOKEN_RES);
	::hoosho::msg::z::GetPAAccessTokenRes* pRespBody = pZMsgRes->mutable_get_pa_access_token_res();
...
void ClientProcessor::process_wx_pay_callback(lce::net::ConnectionInfo& conn, const ::hoosho::msg::Msg& msg)
{
	ExecutorThreadRequestElement stThreadRequest(ExecutorThreadRequestType::T_ORDER_PAY_SUCC
													, FsmContainer<int>::FSM_TYPE_BUSINESS
													, -1
													, "");
	stThreadRequest.m_wx_pay_callback_xml = msg.z_msg_req().wx_pay_callback_ntf().xml();
...
{
	std::string strAuthCode = msg.z_msg_req().code();
	hoosho::msg::z::UserInfo stUserInfo;
	...
	if(USER_LOGIN_STATUS_SUCCESS == iStatus)
	{
		...
		::hoosho::msg::z::MsgRes* pZMsgPes = stRespMsg.mutable_z_msg_res();
		pZMsgPes->set_sub_cmd(::hoosho::msg::z::USER_LOGIN_RES);
		::hoosho::msg::z::UserInfo* pUserInfo = pZMsgPes->add_user_info_list();
...
{		
	...
	//check login state
	const ::hoosho::msg::z::MsgReq& stZMsgReq = msg.z_msg_req();
	std::string strAuthCode = stZMsgReq.code();
	hoosho::msg::z::UserInfo stUserInfo;
	...
	//check phone vc
	if(stZMsgReq.sub_cmd() == ::hoosho::msg::z::SET_PHONE_REQ)
	{
		std::string strPhone = stZMsgReq.set_phone_req().phone();	
		std::string strPhoneVC = stZMsgReq.set_phone_req().vc();	
...
void ClientProcessor::add_login_session(const std::string& strAuthCode, const hoosho::msg::z::UserInfo& stUserInfo, uint64_t qwExpireTs)
{
...
int ClientProcessor::check_login_session(const std::string& strAuthCode, hoosho::msg::z::UserInfo& stUserInfo)
{
	LoginSessionInfoMapIterator iter = _login_session_map.find(strAuthCode);
	if(iter == _login_session_map.end())
	{
		LOG4CPLUS_DEBUG(logger, "login session not exists");
	...
	if(stLoginSessionInfo._expire_ts != DATE21160101)
