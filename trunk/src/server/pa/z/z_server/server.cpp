...
#define s_env_home_path "Z_SERVER_HOME_PATH"
...
int ZServer::impl_init()
{
    ...
    for(int i=0; i<stConfig.get_int_param("THREAD_QUEUE", "size"); i++)
    {
        ...
        ExecutorThreadOrder* threadOrder = new ExecutorThreadOrder();
        if(threadOrder->init(g_executor_thread_processor->get_order_queue(i)) <0)
        {
            return -1;
        }
        threadOrder->start();
