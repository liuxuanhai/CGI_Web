...
class CgiWXMediaVoiceUpload: public HooshoCgiCard
{
public:
	CgiWXMediaVoiceUpload() :
	...
	bool InnerProcess()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			iRet = WXHttpsReq::WXAPIUploadMedia(strMediaData, strMediaName, "voice", strMyAccessToken, strMediaId, intCreateTime, strErrMsg);
			if (iRet != 0)
			{
				if (iRet == 40001)
				{
		...
		GetAnyValue()["voice_media_id"] = strMediaId;
