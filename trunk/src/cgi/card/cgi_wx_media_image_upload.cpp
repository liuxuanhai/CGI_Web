...
class CgiWXMediaImageUpload: public HooshoCgiCard
{
public:
	CgiWXMediaImageUpload() :
	...
	bool InnerProcess()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			iRet = WXHttpsReq::WXAPIUploadMedia(strMediaData, strMediaName, "image", strMyAccessToken, strMediaId, intCreateTime, strErrMsg);
			if (iRet != 0)
			{
				if (iRet == 40001)
				{
		...
		GetAnyValue()["image_media_id"] = strMediaId;
