...
class CgiWXMediaVideoUpload: public HooshoCgiCard
{
public:
	CgiWXMediaVideoUpload() :
	...
	bool InnerProcess()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			iRet = WXHttpsReq::WXAPIUploadMedia(strMediaData, strMediaName, "video", strMyAccessToken, strMediaId, intCreateTime, strErrMsg);
			if (iRet != 0)
			{
				if (iRet == 40001)
				{
		...
		GetAnyValue()["video_media_id"] = strMediaId;
