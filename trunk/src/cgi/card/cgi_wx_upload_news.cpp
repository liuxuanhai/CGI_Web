...
class CgiWXUploadNews: public HooshoCgiCard
{
public:
	CgiWXUploadNews() :
	...
	bool InnerProcess()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			iRet = WXHttpsReq::WXAPIUploadNews(strMyAccessToken, strPostData, strMediaid, strErrMsg);
			if (iRet != 0)
			{
				if (iRet == 40001)
				{
				...
				strErrMsg = "WXHttpsReq::WXAPIUploadNews failed, errmsg=" + strErrMsg;
