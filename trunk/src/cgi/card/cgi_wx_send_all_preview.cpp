...
class CgiWXSendAllPreview: public HooshoCgiCard
{
public:
	CgiWXSendAllPreview() :
	...
	bool InnerProcess()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			iRet = WXHttpsReq::WXAPISendAllPreview(strMyAccessToken, strPostData, strMsgid, strErrMsg);
			if (iRet != 0)
			{
				if (iRet == 40001)
				{
				...
				strErrMsg = "WXHttpsReq::WXAPISendAllPreview failed, errmsg=" + strErrMsg;
		...
		LOG4CPLUS_DEBUG(logger, "msg_id="<<strMsgid);
