...
class CgiUserFansGet: public lce::cgi::Cgi
{
public:
	 CgiUserFansGet(uint32_t qwFlag, const std::string& strCgiConfigFile, const std::string& strLogConfigFile)
...
public:
	...
	virtual bool Process()
	{
		...
		::hoosho::msg::Msg stRequest;
		::hoosho::msg::MsgHead* pHead = stRequest.mutable_head();
		pHead->set_cmd(::hoosho::msg::QUERY_USER_FANS_REQ);
		pHead->set_seq(time(0));
		::hoosho::user::QueryUserFansReq* pQueryUserFansReq = stRequest.mutable_query_user_fans_req();
		pQueryUserFansReq->set_openid_md5(qwOpenidMd5);
		...
		//parse response
		const ::hoosho::msg::MsgHead& stHead = stResponse.head();
		if(stHead.cmd() != ::hoosho::msg::QUERY_USER_FANS_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
			LOG4CPLUS_ERROR(logger, "response.cmd="<<stHead.cmd()<<", unknown, fuck!!!");
			return true;	
		...
		for(int i=0; i<stResponse.query_user_fans_res().openid_md5_list_size(); ++i)
		{
			std::string strOpenidMd5 = int_2_str(stResponse.query_user_fans_res().openid_md5_list(i));
...
IMPL_LOGGER(CgiUserFansGet, logger);

int main(int argc, char** argv)
{
	CgiUserFansGet cgi(lce::cgi::Cgi::FLAG_POST_ONLY, "config.ini", "logger.properties");
    if (!cgi.Run())
    {
        return -1;
    }
