...
public:
							...
							uint64_t iOpenidMd5)
	{
		...
		::common::protoio::ProtoIOTcpClient ioclient(feedServerIP, feedServerPort);
	...
	bool InnerProcess()
	{
		...
		if (iPaAppidMd5 == 0 || iOpenidMd5 == 0)
		{
			DoReply(CGI_RET_CODE_INVALID_PARAM);
			LOG4CPLUS_ERROR(logger, "invalid iPaAppidMd5="<<iPaAppidMd5<<",or iOpenidMd5="<<iOpenidMd5);
			return true;
		}
		...
		::common::protoio::ProtoIOTcpClient client(msgServerIP, msgServerPort);
		std::string strErrMsg = "";
		::hoosho::msg::Msg stRequestMsgServer;
		::hoosho::msg::Msg stResponseMsgServer;

		::hoosho::msg::MsgHead* headerNoticeServer = stRequestMsgServer.mutable_head();
		uint32_t iSeq = 1;
		headerNoticeServer->set_seq(iSeq);
		headerNoticeServer->set_cmd(hoosho::msg::QUERY_NOTICE_RECORD_REQ);
		::hoosho::sixin::QueryNoticeRecordReq* req = stRequestMsgServer.mutable_notice_record_query_req();
		...
		if (client.io(stRequestMsgServer, stResponseMsgServer, strErrMsg) < 0)
		{
		...
		if (stResponseMsgServer.head().cmd() != hoosho::msg::QUERY_NOTICE_RECORD_RES || stResponseMsgServer.head().seq() != iSeq)
		{
			LOG4CPLUS_ERROR(logger, "QUERY_NOTICE_RECORD_RES.cmd = "<<stResponseMsgServer.head().cmd()<<", unknown, fuck!!!");
			DoReply(CGI_RET_CODE_SERVER_BUSY);
			return true;

		}
		if (stResponseMsgServer.head().result() == hoosho::msg::E_SERVER_INNER_ERROR)
		{
			LOG4CPLUS_ERROR(logger, "QUERY_NOTICE_RECORD_RES.result = E_SERVER_INNER_ERROR" << std::endl);
			DoReply(CGI_RET_CODE_SERVER_BUSY);
			return true;
		}
		if (stResponseMsgServer.head().result() != hoosho::msg::E_OK)
		{
		...
		LOG4CPLUS_DEBUG(logger, "QUERY_NOTICE_RECORD_RES Msg: \n" << stResponseMsgServer.Utf8DebugString());

		if (iType == hoosho::commenum::NOTICE_RECORD_TYPE_COMMENT)
		{					
			set<uint64_t> setFollowId;
			
			const hoosho::sixin::QueryNoticeRecordRes& res = stResponseMsgServer.notice_record_query_res();
			...
			for (int i = 0; i < iSize; i++)
			{
				...
				stNoticeRecord["create_ts"] = int_2_str(res.notice_record_list(i).create_ts());
		...
		else if (iType == hoosho::commenum::NOTICE_RECORD_TYPE_FAVORITE)
		{
			set<uint64_t> setOpenidMd5;
			set<uint64_t> setFeedId;			
		...
		else if (iType == hoosho::commenum::NOTICE_RECORD_TYPE_SYSTEM_MSG)	//系统消息只返回邋notice_record_list
		{
			const hoosho::sixin::QueryNoticeRecordRes& res = stResponseMsgServer.notice_record_query_res();
			int iSize = res.notice_record_list_size();
			...
			for (int i = 0; i < iSize; i++)
			{
				...
				stNoticeRecord["pa_appid_md5"] = int_2_str(res.notice_record_list(i).pa_appid_md5());
				stNoticeRecord["openid_md5"] = int_2_str(res.notice_record_list(i).openid_md5());	//type=5 ,openid_md5=0
				stNoticeRecord["create_ts"] = int_2_str(res.notice_record_list(i).create_ts());
				stNoticeRecord["type"] = res.notice_record_list(i).type();
				stNoticeRecord["status"] = res.notice_record_list(i).status();
				stNoticeRecord["extra_data_0"] = res.notice_record_list(i).extra_data_0();	//type=5, 系统消息标题
				stNoticeRecord["extra_data_1"] = res.notice_record_list(i).extra_data_1();	//type=5, 系统消息正文
				stNoticeRecord["extra_data_2"] = res.notice_record_list(i).extra_data_2();	//type=5, NULL
