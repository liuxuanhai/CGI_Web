...
public:
	...
	bool InnerProcess()
	{
		...
		pZMsgReq->set_sub_cmd(::hoosho::msg::z::DEL_MSG_REQ);
		pZMsgReq->set_code(strCode);

		::hoosho::msg::z::DelMsgReq* pDelMsgReq = pZMsgReq->mutable_del_msg_req();
		pDelMsgReq->set_msg_id(qwMsgId);
		pDelMsgReq->set_openid_to(strOpenidTo);		
		...
		if(stZMsgRes.sub_cmd() != ::hoosho::msg::z::DEL_MSG_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
