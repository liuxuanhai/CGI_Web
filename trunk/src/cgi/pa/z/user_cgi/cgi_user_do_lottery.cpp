...
class CgiUserDoLottery: public CgiServer
{
public:
	CgiUserDoLottery(): CgiServer(0, "config.ini", "logger.properties")
	{
	}
	...
	bool InnerProcess()
	{
		...
		pZMsgReq->set_sub_cmd(::hoosho::msg::z::DO_LOTTERY_REQ);
		...
		if(stHead.result() != ::hoosho::msg::E_OK)
		{
			...
			else if(stHead.result() == ::hoosho::msg::E_AVAILABLE_LOTTERY_TIME_NOT_ENOUGH)
				DoReply(CGI_RET_CODE_AVAILABLE_LOTTERY_TIME_NOT_ENOUGH);
			else
				DoReply(CGI_RET_CODE_SERVER_BUSY);
		...
		if(stZMsgRes.sub_cmd() != ::hoosho::msg::z::DO_LOTTERY_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
		...
		//5.feedback
		const ::hoosho::msg::z::DoLotteryRes& stDoLotteryRes = stZMsgRes.do_lottery_res();
		GetAnyValue()["prize"] = stDoLotteryRes.prize();
		GetAnyValue()["prize_desc"] = stDoLotteryRes.prize_desc();
