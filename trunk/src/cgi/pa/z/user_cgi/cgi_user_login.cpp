...
class CgiUserLogin: public CgiServer
{
public:
	CgiUserLogin(): CgiServer(0, "config.ini", "logger.properties")
	{
	}
	...
	bool InnerProcess()
	{
		...
		pZMsgReq->set_sub_cmd(::hoosho::msg::z::USER_LOGIN_REQ);
		...
		if(stZMsgRes.sub_cmd() != ::hoosho::msg::z::USER_LOGIN_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
		...
		//5.feedback
		const ::hoosho::msg::z::UserInfo& stUserInfo = stZMsgRes.user_info_list(0);
		lce::cgi::CAnyValue item;
		UserInfoPB2Any(stUserInfo, item);
		GetAnyValue()["user_info"] = item;	
