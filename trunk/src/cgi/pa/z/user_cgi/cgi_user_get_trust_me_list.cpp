...
class CgiUserGetTrustMeList: public CgiServer
{
public:
	CgiUserGetTrustMeList(): CgiServer(0, "config.ini", "logger.properties")
	{
	}
	...
	bool InnerProcess()
	{
		...
		pZMsgReq->set_sub_cmd(::hoosho::msg::z::GET_TRUST_ME_LIST_REQ);
		pZMsgReq->set_code(strCode);

		::hoosho::msg::z::GetTrustMeListReq* pGetTrustMeListReq = pZMsgReq->mutable_get_trust_me_list_req();
		pGetTrustMeListReq->set_openid(strOpenid);
		pGetTrustMeListReq->set_offset_openid(strOffsetOpenid);
		pGetTrustMeListReq->set_limit(iLimit);
		...
		if(stZMsgRes.sub_cmd() != ::hoosho::msg::z::GET_TRUST_ME_LIST_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
		...
		const ::hoosho::msg::z::GetTrustMeListRes& stGetTrustMeListRes = stZMsgRes.get_trust_me_list_res();
		for(int i=0; i<stGetTrustMeListRes.user_extra_info_list_size(); i++)
		{
			lce::cgi::CAnyValue item, stAnyValue;
			const ::hoosho::msg::z::UserExtraInfo& stUserExtraInfo = stGetTrustMeListRes.user_extra_info_list(i);
		...
		for(int i=0; i < stGetTrustMeListRes.user_trust_sb_status_list_size(); i++)
		{
			lce::cgi::CAnyValue stAnyValue;
			const ::hoosho::msg::z::UserTrustSBStatus& stUserTrustSBStatus = stGetTrustMeListRes.user_trust_sb_status_list(i);
