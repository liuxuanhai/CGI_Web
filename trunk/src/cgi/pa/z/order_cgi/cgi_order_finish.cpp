...
class CgiOrderFinish:public CgiOrderBase
{
public:
	CgiOrderFinish():CgiOrderBase(0, "config.ini", "logger.properties")
	{
	...
	bool InnerProcess()
	{
		...
		pZMsgReq->set_sub_cmd(::hoosho::msg::z::FINISH_ORDER_REQ);
		pZMsgReq->set_code(strCode);

		::hoosho::msg::z::FinishOrderReq* pFinishOrderReq = pZMsgReq->mutable_finish_order_req();
		pFinishOrderReq->set_orderid(qwOrderid);
