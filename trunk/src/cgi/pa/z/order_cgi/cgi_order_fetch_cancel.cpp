...
class CgiOrderFetchCancel:public CgiOrderBase
{
public:
	CgiOrderFetchCancel():CgiOrderBase(0, "config.ini", "logger.properties")
	{
	...
	bool InnerProcess()
	{
		...
		pZMsgReq->set_sub_cmd(::hoosho::msg::z::ORDER_FETCH_CANCEL_REQ);
		pZMsgReq->set_code(strCode);

		::hoosho::msg::z::FetchCancelReq* pFetchCancelReq = pZMsgReq->mutable_fetch_cancel_req();
		pFetchCancelReq->set_orderid(qwOrderid);
