...
class CgiOrderFetchAccept:public CgiOrderBase
{
public:
	CgiOrderFetchAccept():CgiOrderBase(0, "config.ini", "logger.properties")
	{
	...
	bool InnerProcess()
	{
		...
		string strFetchOpenid = GetInput().GetValue("fetch_openid");

		EMPTY_STR_RETURN(strCode);
		ZERO_INT_RETURN(qwOrderid);
		EMPTY_STR_RETURN(strFetchOpenid);
		...
		pZMsgReq->set_sub_cmd(::hoosho::msg::z::ORDER_FETCH_ACCEPT_REQ);
		pZMsgReq->set_code(strCode);

		::hoosho::msg::z::FetchAcceptReq* pFetchAcceptReq = pZMsgReq->mutable_fetch_accept_req();
		pFetchAcceptReq->set_orderid(qwOrderid);
		pFetchAcceptReq->set_slave_openid(strFetchOpenid);
