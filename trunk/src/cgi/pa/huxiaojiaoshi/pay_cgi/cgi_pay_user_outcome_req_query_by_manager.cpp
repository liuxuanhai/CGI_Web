...
class CgiPayOutcomeReqQueryByManager: public CgiPayServer
{
public:
	CgiPayOutcomeReqQueryByManager():
	...
	{
		//1.get HTTP params		
		uint64_t state = strtoul(((string)GetInput().GetValue("state")).c_str(), NULL, 10);		
		...
		if(state > 3 || state < 1)
		{
			...
			LOG4CPLUS_ERROR(logger, "INVALID PARAM state=" << state);
			return true;
		}
		...
		stQueryOutcomeReq->set_openid("");
		stQueryOutcomeReq->set_state(state);
...
{
	CgiPayOutcomeReqQueryByManager cgi;
	if(!cgi.Run())
	{
		return -1;
	}
