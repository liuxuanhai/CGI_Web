...
public:
	...
	virtual bool Process()
	{
		...
		pHead->set_cmd(::hoosho::msg::J_GET_PA_JSAPI_SIGNATURE_REQ);
		pHead->set_seq(time(0));
		::hoosho::j::user::GetPAJSAPISignReq* pGetJSAPISignReq = stRequest.mutable_get_pa_jsapi_sign_req();
		...
		if(stHead.cmd() != ::hoosho::msg::J_GET_PA_JSAPI_SIGNATURE_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
		...
		const ::hoosho::j::user::GetPAJSAPISignRes& stGetJSAPISignRes = stResponse.get_pa_jsapi_sign_res();
