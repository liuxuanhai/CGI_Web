...
public:
	...
	virtual bool Process()
	{
		...
		pHead->set_cmd(::hoosho::msg::S_PROJECT_REQ);
		pHead->set_seq(time(0));
		
		::hoosho::msg::s::MsgReq* pZMsgReq = stRequest.mutable_s_msg_req();
		pZMsgReq->set_sub_cmd(::hoosho::msg::s::GET_PA_JSAPI_SIGNATURE_REQ);
		
		::hoosho::msg::s::GetPAJSAPISignReq* pGetJSAPISignReq = pZMsgReq->mutable_get_pa_jsapi_sign_req();
		...
		::common::protoio::ProtoIOTcpClient stProtoIOTcpClient(m_s_server_ip, m_s_server_port);
		...
		const ::hoosho::msg::s::MsgRes& stSMsgRes = stResponse.s_msg_res();
		if(stSMsgRes.sub_cmd() != ::hoosho::msg::s::GET_PA_JSAPI_SIGNATURE_RES)
		{			
			DoReply(CGI_RET_CODE_SERVER_BUSY);
		...
		const ::hoosho::msg::s::GetPAJSAPISignRes& stGetJSAPISignRes = stSMsgRes.get_pa_jsapi_sign_res();
