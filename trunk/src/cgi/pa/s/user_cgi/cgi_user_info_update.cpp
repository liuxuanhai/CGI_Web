...
public:
	...
	bool InnerProcess()
	{
		...
		pSMsgReq->set_sub_cmd(::hoosho::msg::s::UPDATE_USER_INFO_REQ);
		pSMsgReq->set_code(strCode);

		::hoosho::msg::s::UpdateUserInfoReq* pUpdateUserInfoReq = pSMsgReq->mutable_update_user_info_req();
		...
		if(stSMsgRes.sub_cmd() != ::hoosho::msg::s::UPDATE_USER_INFO_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
