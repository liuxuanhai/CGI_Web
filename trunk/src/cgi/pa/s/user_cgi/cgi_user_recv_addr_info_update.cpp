...
class CgiUserRecvAddrInfoUpdate: public CgiServer
{
public:
	CgiUserRecvAddrInfoUpdate(): CgiServer(0, "config.ini", "logger.properties")
	{
	}
	...
	bool InnerProcess()
	{
		...
		std::string strAddrDistrict = (std::string)GetInput().GetValue("addr_district");
		...
		EMPTY_STR_RETURN(strAddrDistrict);
		...
		pSMsgReq->set_sub_cmd(::hoosho::msg::s::UPDATE_USER_RECV_ADDR_INFO_REQ);
		pSMsgReq->set_code(strCode);

		::hoosho::msg::s::UpdateUserRecvAddrInfoReq* pUpdateUserRecvAddrInfoReq = pSMsgReq->mutable_update_user_recv_addr_info_req();
		pUpdateUserRecvAddrInfoReq->set_set_default_addr(dwDefaultAddr);

		::hoosho::msg::s::UserRecvAddrInfo *pUserRecvAddrInfo = pUpdateUserRecvAddrInfoReq->mutable_user_recv_addr_info();

		pUserRecvAddrInfo->set_addr_num(dwAddrNum);
		...
		pUserRecvAddrInfo->set_addr_district(strAddrDistrict);
		...
		if(stSMsgRes.sub_cmd() != ::hoosho::msg::s::UPDATE_USER_RECV_ADDR_INFO_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
