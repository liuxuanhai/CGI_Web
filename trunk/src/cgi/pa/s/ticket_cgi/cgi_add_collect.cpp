...
class CgiAddCollect: public CgiServer
{
public:
	CgiAddCollect(): CgiServer(0, "config.ini", "logger.properties")
	{
	}
	...
	bool InnerProcess()
	{
		...
		pSMsgReq->set_sub_cmd(::hoosho::msg::s::ADD_COLLECT_REQ);
		pSMsgReq->set_code(strCode);

		::hoosho::msg::s::AddCollectReq* pAddCollectReq = pSMsgReq->mutable_add_collect_req();
		::hoosho::msg::s::CollectInfo* pCollectInfo = pAddCollectReq->mutable_collect_info();
		pCollectInfo->set_feed_id(qwFeedid);
		...
		if(stSMsgRes.sub_cmd() != ::hoosho::msg::s::ADD_COLLECT_RES)
		{
			DoReply(CGI_RET_CODE_SERVER_BUSY);
