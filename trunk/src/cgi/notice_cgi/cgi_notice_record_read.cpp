...
class CgiNoticeRecordRead: public NoticeServerCgi
{
public:
	CgiNoticeRecordRead() :
	...
	bool InnerProcess()
	{
		...
		uint32_t iSeq = 1;
		header->set_seq(iSeq);
		header->set_cmd(hoosho::msg::READ_NOTICE_RECORD_REQ);
		::hoosho::noticenotify::ReadNoticeRecordReq* req = stRequest.mutable_notice_record_read_req();
		...
		pBodyNoticeRecord->set_status(1);
		...
		if (stResponse.head().cmd() == hoosho::msg::READ_NOTICE_RECORD_RES && stResponse.head().seq() == iSeq)
		{
